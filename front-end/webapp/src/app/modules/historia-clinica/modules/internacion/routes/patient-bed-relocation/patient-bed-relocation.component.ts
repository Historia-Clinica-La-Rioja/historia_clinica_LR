...
import { scrollIntoError, hasError, futureTimeValidation } from '@core/utils/form.utils';
import { ConfirmDialogComponent } from '@core/dialogs/confirm-dialog/confirm-dialog.component';
import { ContextService } from '@core/services/context.service';
import { BedService } from '@api-rest/services/bed.service';
import { newMoment, momentFormat, DateFormat, momentParseDateTime, momentParseDate } from '@core/utils/moment.utils';
...
export class PatientBedRelocationComponent implements OnInit {
	...
	ngOnInit(): void {
		...
		this.form = this.formBuilder.group({
			...
			relocationTime: [momentFormat(this.today, DateFormat.HOUR_MINUTE), [Validators.required, futureTimeValidation]],
		...
		this.form.controls['relocationDate'].valueChanges.subscribe(
			(selectedDate: Moment) => {
				...
				if (selectedDateString === todayDateString) {
					this.form.controls['relocationTime'].setValidators([Validators.required, futureTimeValidation]);
					this.form.controls['relocationTime'].updateValueAndValidity();
				} else {
					this.form.controls['relocationTime'].setValidators(Validators.required);
					this.form.controls['relocationTime'].updateValueAndValidity();
				}
			}
		);
