...
export class PatientGenderService {
	...
	getOptionalGender(patientGender: string, selfPerceivedGender: string) {
		const gender = this.getPatientGender(patientGender, selfPerceivedGender);
		return gender ? gender : 'Sin género';
	}
}
