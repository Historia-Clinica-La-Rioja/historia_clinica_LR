...
export class MonthlyReportComponent implements OnInit {
	...
	private queueReport({reportId, reportDescription, reportFilters}) {
		...
		if (reportId === REPORT_TYPES_ID.GUARD_ATTENTION_DETAIL_REPORT) {
			return this.monthlyQueueService.showDialog(
				InstitutionReportType.EmergencyCareNominalDetail,
				reportFilters,
				fileName
			)
		}
		if (reportId === REPORT_TYPES_ID.NOMINAL_DIAGNOSTIC_IMAGING) {
			return this.monthlyQueueService.showDialog(
				InstitutionReportType.ImageNetworkProductivity,
				reportFilters,
				fileName
			)
		}
