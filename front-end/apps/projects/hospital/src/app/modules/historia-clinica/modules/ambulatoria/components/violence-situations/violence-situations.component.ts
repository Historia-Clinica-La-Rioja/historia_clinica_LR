...
import { ApiErrorDto, CoordinationInsideHealthSectorDto, CoordinationOutsideHealthSectorDto, EAggressorRelationship, EHealthInstitutionOrganization, EHealthSystemOrganization, EInstitutionReportPlace, EViolenceTowardsUnderageType, InstitutionReportDto, PageDto, SexualViolenceDto, SnomedDto, VictimKeeperReportDto, ViolenceEpisodeDetailDto, ViolenceReportAggressorDto, ViolenceReportDto, ViolenceReportImplementedActionsDto, ViolenceReportSituationDto, ViolenceReportSituationEvolutionDto, ViolenceReportVictimDto } from '@api-rest/api-model';
...
export class ViolenceSituationsComponent implements OnInit {
	...
	constructor(private readonly dockPopupService: DockPopupService, 
				...
				private violenceReportService: ViolenceReportService,
	...
	seeDetails = (ids: SelectableCardIds) => {
		this.violenceReportService.getEvolutionData(this.patientId, ids.id, ids.relatedId)
			.subscribe({
				next: (result: ViolenceReportDto) => this.mapToDetailedInformation(result, ids),
				error: (error: ApiErrorDto) => this.snackbarService.showError(error.message)
			})
	}

	private mapToDetailedInformation(result: ViolenceReportDto, ids: SelectableCardIds) {
		this.selectedViolenceEvolution = {
			id: null,
			title: `Situación #${ids.id} | Evolución ${ids.relatedId}`,
			oneColumn: [
				{
					icon: 'cancel',
		...
		result.observation ? this.selectedViolenceEvolution.oneColumn.push(
			{
				icon: 'cancel',
	...
	private buildViolentPersonInformation = (aggressorData: ViolenceReportAggressorDto[]): string[] => {
		...
		aggressorData.forEach((aggressor: ViolenceReportAggressorDto) => {
			...
			array.push(aggressor.aggressorData.actorPersonalData.municipality.description);
	...
	private buildLackOfLegalCapacity = (array: string[], victimData: ViolenceReportVictimDto) => {
		if (victimData.lackOfLegalCapacity) {
			...
			array.push(victimData.keeperData.actorPersonalData.municipality.description);
