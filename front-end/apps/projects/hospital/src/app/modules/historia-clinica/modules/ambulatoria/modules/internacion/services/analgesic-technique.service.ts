...
export class AnalgesicTechniqueService {
	...
	setData(techniques: AnalgesicTechniqueDto[]) {
        this.analgesicTechniqueList = techniques.map(technique => ({
            snomed: technique.snomed,
            dosis: technique.dosage?.quantity?.value,
            unit: technique.dosage?.quantity?.unit,
            cateter: technique.catheter,
            cateterNote: technique.catheterNote,
            injectionNote: technique.injectionNote,
        }));
        this.dataEmitter.next(this.analgesicTechniqueList);
    }
