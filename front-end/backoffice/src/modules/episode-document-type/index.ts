...
const episodesDocumentTypes = (permissions: SGXPermissions) => ({
    ...
    create: permissions?.roleAssignments?.filter(roleAssignment => (roleAssignment.role === ADMINISTRADOR_INSTITUCIONAL_BACKOFFICE.role 
        || roleAssignment.role === ADMINISTRADOR.role)).length > 0 ? EpisodeDocumentTypeCreate : undefined,
