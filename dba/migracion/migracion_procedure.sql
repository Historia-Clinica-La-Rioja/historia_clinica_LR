...
BEGIN
    ...
    FOR temprow IN
        SELECT s.pt, s.sctid, s.parent_fsn, s.parent_id
    FROM public.snomed AS s
        LOOP
            IF (NOT EXISTS (SELECT 1 FROM snomed WHERE sctid = temprow.sctid  AND pt = temprow.pt)) THEN
    INSERT INTO snomed (sctid, pt, parent_id, parent_fsn) VALUES (temprow.sctid, temprow.pt, temprow.parent_id, temprow.parent_fsn);
    END IF;
    END LOOP;

    INSERT INTO migrate.health_condition (id, patient_id, sctid_code, status_id, verification_status_id, start_date,
    ...
    SELECT (-offset_value-hc.id), (-offset_value-patient_id), null, status_id, verification_status_id,
           ...
           (-offset_value-deleted_by), deleted_on
    FROM public.health_condition AS hc
             JOIN public.snomed AS s ON hc.snomed_id = s.id
             JOIN migrate.snomed AS ms ON (ms.sctid = s.sctid AND ms.pt = s.pt);

    INSERT INTO migrate.observation_lab (id, patient_id, sctid_code, status_id, category_id, "value", effective_time,
    ...
    SELECT (-offset_value-hc.id), (-offset_value-patient_id), null, status_id, category_id,
           ...
           (-offset_value-deleted_by), deleted_on
    FROM public.observation_lab AS hc
             JOIN public.snomed AS s ON hc.snomed_id = s.id
             JOIN migrate.snomed AS ms ON (ms.sctid = s.sctid AND ms.pt = s.pt);

    INSERT INTO migrate.observation_vital_sign (id, patient_id, sctid_code, status_id, category_id, "value", effective_time,
                                                note_id, created_by, updated_by, created_on, updated_on, cie10_codes, snomed_id,
                                                deleted, deleted_by, deleted_on)
    SELECT (-offset_value-hc.id), (-offset_value-patient_id), null, status_id, category_id,
           hc.value, effective_time, (-offset_value-note_id), (-offset_value-created_by),
           (-offset_value-updated_by), created_on, updated_on, cie10_codes, ms.id, deleted,
           (-offset_value-deleted_by), deleted_on
    FROM public.observation_vital_sign AS hc
             JOIN public.snomed AS s ON hc.snomed_id = s.id
             JOIN migrate.snomed AS ms ON (ms.sctid = s.sctid AND ms.pt = s.pt);

    INSERT INTO migrate.procedures (id, patient_id, sctid_code, status_id, note_id, performed_date, created_by, updated_by,
    ...
    SELECT (-offset_value-hc.id), (-offset_value-patient_id), null, status_id, (-offset_value-note_id), performed_date,
           ...
           (-offset_value-deleted_by), deleted_on
    FROM public.procedures AS hc
    JOIN public.snomed AS s ON hc.snomed_id = s.id
    JOIN migrate.snomed AS ms ON (ms.sctid = s.sctid AND ms.pt = s.pt);

    INSERT INTO migrate.allergy_intolerance (id, patient_id, sctid_code, status_id, verification_status_id, start_date,
    ...
    SELECT (-offset_value-hc.id), (-offset_value-patient_id), null, status_id, verification_status_id, start_date,
           ...
           (-offset_value-deleted_by), deleted_on
    FROM public.allergy_intolerance AS hc
             JOIN public.snomed AS s ON hc.snomed_id = s.id
             JOIN migrate.snomed AS ms ON (ms.sctid = s.sctid AND ms.pt = s.pt);

    INSERT INTO migrate.diagnostic_report (id, patient_id, status_id, health_condition_id, effective_time, link, note_id,
    ...
    SELECT (-offset_value-hc.id), (-offset_value-patient_id), status_id, (-offset_value-health_condition_id), effective_time,
           ...
           (-offset_value-deleted_by), deleted_on
    FROM public.diagnostic_report AS hc
             JOIN public.snomed AS s ON hc.snomed_id = s.id
             JOIN migrate.snomed AS ms ON (ms.sctid = s.sctid AND ms.pt = s.pt);

    FOR temprow IN
    SELECT (-offset_value-hc.id) AS id, (-offset_value-patient_id) as patient_id, null, status_id,
    ...
    FROM public.inmunization AS hc
         JOIN public.snomed AS s ON hc.snomed_id = s.id
         JOIN migrate.snomed AS ms ON (ms.sctid = s.sctid AND ms.pt = s.pt)
    LOOP
        INSERT INTO migrate.inmunization (id, patient_id, sctid_code, status_id, institution_id, administration_date,
        ...
        VALUES (temprow.id, temprow.patient_id, null, temprow.status_id, temprow.institution_id, temprow.administration_date,
            temprow.expiration_date, temprow.note_id, temprow.created_by, temprow.updated_by, temprow.created_on,
            temprow.updated_on, temprow.cie10_codes,temprow.snomed_id, temprow.deleted, temprow.deleted_by,
            temprow.deleted_on,
            (SELECT ms.id AS snomed_commercial_id
            FROM public.snomed AS s
            JOIN migrate.snomed AS ms ON (ms.sctid = s.sctid AND ms.pt = s.pt)
            WHERE temprow.snomed_commercial_id = s.id),
            temprow.scheme_id, temprow.condition_id, temprow.dose_order,
            temprow.lot_number, temprow.billable, temprow.dose, temprow.doctor_info, temprow.institution_info);
    END LOOP;


    INSERT INTO migrate.dosage (id, sequence, "count", duration, duration_unit, frequency, period_unit, dose_quantity_id,
    ...
    SELECT (-offset_value-id), sequence, count, duration, duration_unit, frequency, period_unit, dose_quantity_id,
           chronic, start_date, end_date, suspended_start_date, suspended_end_date, event
    FROM public.dosage AS d;


    INSERT INTO migrate.medication_statement (id, patient_id, sctid_code, status_id, note_id, created_by, updated_by,
    ...
    SELECT (-offset_value-m.id), (-offset_value-patient_id), null, status_id,
           ...
           (-offset_value-deleted_by), deleted_on
    FROM public.medication_statement AS m
    JOIN public.snomed AS s ON m.snomed_id = s.id
    JOIN migrate.snomed AS ms ON (ms.sctid = s.sctid AND ms.pt = s.pt);
