...
public class UpdateReferenceRegulationState {
	...
	public void run(Integer referenceId, Short stateId, String reason){
		...
		if (stateId.equals(EReferenceRegulationState.AUDITED.getId()))
			changeAdministrativeState(referenceId);
	...
	private void validateStateChange(Short stateId, String reason){
		...
		if (regulationState.equals(EReferenceRegulationState.WAITING_AUDIT))
		...
		if (regulationState.equals(EReferenceRegulationState.AUDITED) && reason != null)
	...
	private void assertContextValid(Integer referenceId) {
		var regulationStateId = referenceStorage.findById(referenceId).map(Reference::getRegulationStateId).orElse(null);
		if (regulationStateId == null || !regulationStateId.equals(EReferenceRegulationState.WAITING_AUDIT.getId()))
			throw new UpdateReferenceRegulationStateException(UpdateReferenceRegulationStateExceptionEnum.INVALID_STATUS_CHANGE, "No es posible cambiar el estado de la referencia");
	...
	private void changeAdministrativeState(Integer referenceId){
		if (referenceStorage.findById(referenceId).map(Reference::getDestinationInstitutionId).isPresent())
