...
public interface MedicationStatementRepository extends SGXAuditableEntityJPARepository<MedicationStatement, Integer> {
	...
	@Transactional
	@Modifying
	@Query("UPDATE MedicationStatement ms " +
			"SET ms.patientId = :newPatientId " +
			"WHERE ms.id IN :msIds")
	void updatePatient(@Param("msIds")List<Integer> msIds, @Param("newPatientId") Integer newPatientId);
