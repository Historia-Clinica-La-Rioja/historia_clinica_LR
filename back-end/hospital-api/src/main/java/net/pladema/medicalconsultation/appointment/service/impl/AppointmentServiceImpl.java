...
public class AppointmentServiceImpl implements AppointmentService {
    ...
    public Optional<AppointmentBo> getAppointment(Integer appointmentId) {
        ...
        if (featureFlagsService.isOn(AppFeature.HABILITAR_RECURRENCIA_EN_DESARROLLO)) {
            if (Objects.equals(appointmentRepository.recurringAppointmentQuantityByParentId(appointmentId), NO_CHILD_APPOINTMENTS)) {
                result.ifPresent(appointmentBo -> appointmentBo.setHasAppointmentChilds(false));
            } else {
                result.ifPresent(appointmentBo -> appointmentBo.setHasAppointmentChilds(true));
            }
        }
        log.debug(OUTPUT, result);
        return result;
    ...
    public AppointmentTicketImageBo getAppointmentImageTicketData(Integer appointmentId, boolean isTranscribed) {
        ...
        if (result.getMedicalCoverage() == null && result.getMedicalCoverageAcronym() == null) {
            this.appointmentRepository.getMedicalCoverageOrderByAppointment(appointmentId)
                    .ifPresent(medicalCoverageAppoinmentOrderBo -> {
                        result.setMedicalCoverage(medicalCoverageAppoinmentOrderBo.getMedicalCoverage());
                        result.setMedicalCoverageAcronym(medicalCoverageAppoinmentOrderBo.getMedicalCoverageAcronym());
                    });
        }
    ...
    public Boolean hasOldAppointmentWithMinDateLimit(Integer patientId, Integer healthcareProfessionalId, Short minDateLimit) {
        ...
        LocalDate fiveDaysAgo = LocalDate.now().minusDays(minDateLimit);
    ...
    @Override
    public void updateAppointmentByOptionId(AppointmentBo currentAppointment, AppointmentBo newAppointment) {
        log.debug("Input parameters -> currentAppointment {}, newAppointment {}", currentAppointment, newAppointment);
        if (Objects.equals(newAppointment.getAppointmentOptionId(), RecurringAppointmentOption.CURRENT_APPOINTMENT.getId()))
        ...
        if (Objects.equals(newAppointment.getAppointmentOptionId(), RecurringAppointmentOption.CURRENT_AND_NEXT_APPOINTMENTS.getId()))
            updateRecurringAppointmentDate(currentAppointment, newAppointment);

        if (Objects.equals(newAppointment.getAppointmentOptionId(), RecurringAppointmentOption.ALL_APPOINTMENTS.getId())) {
