...
public class EmergencyCareEpisodeStateServiceImpl implements EmergencyCareEpisodeStateService {
	...
	private void assertEpisodeAttend(Integer episodeId) {
		...
		boolean existsActiveEpisode = emergencyCareEpisodeRepository.isEpisodeMedicalOrAdministrativeDischarge(episodeId);
		if (existsActiveEpisode)
			throw new ValidationException("El episodio ya fue dado de alta");
	}
}
