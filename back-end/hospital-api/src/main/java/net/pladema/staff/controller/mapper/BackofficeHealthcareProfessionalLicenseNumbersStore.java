...
@Service
public class BackofficeHealthcareProfessionalLicenseNumbersStore implements BackofficeStore<ProfessionalLicenseNumberDto, Integer> {

    private final ProfessionalLicenseNumberRepository professionalLicenseNumberRepository;

	public BackofficeHealthcareProfessionalLicenseNumbersStore(ProfessionalLicenseNumberRepository professionalLicenseNumberRepository) {
    ...
    public Page<ProfessionalLicenseNumberDto> findAll(ProfessionalLicenseNumberDto example, Pageable pageable) {
		example.setHealthcareProfessionalSpecialtyId(null);
    ...
    public List<ProfessionalLicenseNumberDto> findAll() {
        return professionalLicenseNumberRepository.findAllHealthProfessionalRegistrationNumbers()
				.stream()
				.map(this::fromEntity)
    ...
    public List<ProfessionalLicenseNumberDto> findAllById(List<Integer> ids) {
        return professionalLicenseNumberRepository.findAllById(ids)
				.stream()
				.filter(professionalLicenseNumber -> professionalLicenseNumber.getHealthcareProfessionalSpecialtyId() == null)
				.map(this::fromEntity)
				.collect(Collectors.toList());
    }
