...
import net.pladema.questionnaires.familybg.getpdf.domain.service.PrintFamilyBgService;

@RestController
@Validated
@RequestMapping("/institution/patient/outpatient/consultation/")
public class GetFamilyBgPDFController implements GetFamilyBgPDFAPI {

	public static final String OUTPUT = "Output -> {}";

	private static final Logger logger = LoggerFactory.getLogger(GetFamilyBgPDFController.class);

	private final PrintFamilyBgService printQuestionnaireService;

	private final PdfService pdfService;

	public GetFamilyBgPDFController(PrintFamilyBgService printQuestionnaireService, PdfService pdfService) {
	...
	private ResponseEntity<InputStreamResource> generatedPDFResponse(Map<String, Object> context, String outputFileName) throws IOException {
		logger.debug("input parameters -> context: {}, outputFileName: {}", "familybg_reports", outputFileName);
		FileContentBo fileContentBo = pdfService.generate("familybg_reports", context);
