...
@Service
public class ExcelServiceNRImpl implements ExcelServiceNR {
	...
	@Override
	public IWorkbook buildExcelHospitalizationNursing(String tittle, String[] headers, List<HospitalizationNursing> result) {
	...
	@Override
	public IWorkbook buildExcelNursingEmergencies(String tittle, String[] headers, List<NursingEmergencies> result) {
		IWorkbook wb = WorkbookCreator.createExcelWorkbook();
		createCellStyle(wb);
	...
	@Override
	public IWorkbook buildExcelVaccinesNursing(String title, String[] headers, List<VaccinesNursing> result, String startDate, String endDate) {
		IWorkbook wb = WorkbookCreator.createExcelWorkbook();
		createCellStyle(wb);
		...
		for (VaccinesNursing vaccine : result) {
			institution = vaccine.getInstitution();
			break;
		}

		fillRow(sheet, getHeaderDataDates(headers, title, startDate, endDate, institution));
	...
	private List<CellContent> getHeaderDataDates(String[] subtitles, String title, String startDate, String endDate, String institution) {
		...
		nRow++;
		data.add(new CellContent(nRow, 0, 1, 2, "Período: " + startDate + " al " + endDate, basicStyle));
	...
	private void fillRowContent(IRow row, HospitalizationNursing content, ICellStyle style){
	...
	private void fillRowContent(IRow row, VaccinesNursing content, ICellStyle style) {
		AtomicInteger rowNumber = new AtomicInteger(0);
