...
public class AnnexReportRepositoryImpl implements AnnexReportRepository {
    ...
    public Optional<AnnexIIOutpatientVo> getConsultationAnnexInfo(Long documentId) {
        String query = "WITH t AS (" +
                "       SELECT oc.id, d.id as doc_id, oc.start_date, oc.institution_id, oc.patient_id, oc.clinical_specialty_id, oc.patient_medical_coverage_id, d.created_on, oc.doctor_id " +
                "       FROM {h-schema}document AS d " +
                "       JOIN {h-schema}outpatient_consultation AS oc ON (d.source_id = oc.id  AND d.source_type_id = 1)" +
                "       WHERE d.id = :documentId " +
                "       UNION ALL " +
                "       SELECT vc.id, d.id as doc_id, vc.performed_date as start_date, vc.institution_id, vc.patient_id, vc.clinical_specialty_id, vc.patient_medical_coverage_id, d.created_on, vc.doctor_id " +
                "       FROM {h-schema}document AS d " +
