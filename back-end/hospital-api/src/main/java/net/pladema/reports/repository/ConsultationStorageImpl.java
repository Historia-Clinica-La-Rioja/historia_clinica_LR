...
public class ConsultationStorageImpl implements ConsultationStorage {
    ...
    public List<ConsultationsVo> fetchAllByPatientId(Integer patientId) {
        String sqlString = "WITH t AS ("
                ...
                +  "JOIN document AS d ON (d.source_id = oc.id AND d.source_type_id = 1)"
                ...
                +  "JOIN document AS d ON (d.source_id = vc.id AND d.source_type_id = 5)"
                ...
                +"  FROM t "
                +"  LEFT JOIN clinical_specialty AS cs ON (cs.id = t.clinical_specialty_id) "
                +"  JOIN healthcare_professional AS hp ON (hp.id = t.doctor_id) "
                +"  JOIN person AS pe ON (pe.id = hp.person_id) "
