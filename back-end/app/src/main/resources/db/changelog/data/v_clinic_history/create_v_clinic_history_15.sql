...
BEGIN
    ...
    Drop v_clinic_history

    #####################*/
    DROP VIEW v_clinic_history;
    ...
    SELECT d.id AS id,
           ...
           coalesce('Signos vitales durante la anestesia: ' ||
                    ...
                    (SELECT '\hr' || string_agg('Medición' ||
                                                ...
                                                'Saturación O2: ' || CASE WHEN mp.o2_saturation IS NOT NULL THEN mp.blood_pressure_max || '%' ELSE '-' END || ' ' ||
                                                'End Tidal CO2: ' || CASE WHEN mp.co2_end_tidal IS NOT NULL THEN mp.blood_pressure_max || '' ELSE '-' END, '\n\hr') || '\n'
                     FROM measuring_point mp
                     WHERE mp.document_id = d.id
                     GROUP BY mp.document_id), '') AS vital_signs_anesthesia,
           coalesce('Estado al finalizar la anestesia: ' ||
                    (SELECT CASE WHEN dpas.intentional_sensitivity IS NOT NULL THEN '\n- Sensibilidad dolorosa: ' || CASE WHEN dpas.intentional_sensitivity = TRUE THEN 'Si' ELSE 'No' END END ||
                            CASE WHEN dpas.corneal_reflex IS NOT NULL THEN '\n- Reflejo corneal: ' || CASE WHEN dpas.corneal_reflex = TRUE THEN 'Si' ELSE 'No' END END ||
                            CASE WHEN dpas.obey_orders IS NOT NULL THEN '\n- Obedece órdenes: ' || CASE WHEN dpas.obey_orders = TRUE THEN 'Si' ELSE 'No' END END ||
                            CASE WHEN dpas.talk IS NOT NULL THEN '\n- Conversa: ' || CASE WHEN dpas.talk = TRUE THEN 'Si' ELSE 'No' END END ||
                            CASE WHEN dpas.respiratory_depression IS NOT NULL THEN '\n- Depresión respiratoria: ' || CASE WHEN dpas.respiratory_depression = TRUE THEN 'Si' ELSE 'No' END END ||
                            CASE WHEN dpas.circulatory_depression IS NOT NULL THEN '\n- Depresión circulatoria: ' || CASE WHEN dpas.circulatory_depression = TRUE THEN 'Si' ELSE 'No' END END ||
                            CASE WHEN dpas.vomiting IS NOT NULL THEN '\n- Vómitos: ' || CASE WHEN dpas.vomiting = TRUE THEN 'Si' ELSE 'No' END END ||
                            CASE WHEN dpas.curated IS NOT NULL THEN '\n- Curarizado: ' || CASE WHEN dpas.curated = TRUE THEN 'Si' ELSE 'No' END END ||
                            CASE WHEN dpas.tracheal_cannula IS NOT NULL THEN '\n- Cánula traqual: ' || CASE WHEN dpas.tracheal_cannula = TRUE THEN 'Si' ELSE 'No' END END ||
                            CASE WHEN dpas.pharyngeal_cannula IS NOT NULL THEN '\n- Cánula faringea: ' || CASE WHEN dpas.pharyngeal_cannula = TRUE THEN 'Si' ELSE 'No' END END ||
                            CASE WHEN dpas.internment IS NOT NULL THEN '\n- Se interna: ' || CASE WHEN dpas.internment = TRUE THEN 'Si' ELSE 'No' END
                                || CASE WHEN dpas.internment_place_id IS NOT NULL THEN ' - ' || CASE WHEN dpas.internment_place_id = 1 THEN 'En piso' ELSE 'En unidad de terapia intensiva' END END END ||
                     ...
                     WHERE dpas.document_id = d.id
                       AND d.type_id IN (20)), '') AS post_anesthesia_status
