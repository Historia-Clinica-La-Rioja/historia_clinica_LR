...
public class FetchSuppliesByActivityTest {
    ...
    void procedureSuccess() {
        ...
        when(activityInfoStorage.getSuppliesByActivity(refsetCode, provinceCode, activityId)).thenReturn(
                Collections.singletonList(new SnomedBo("1", "1"))
        );

        List<SnomedBo> result = fetchSuppliesByActivity.run(refsetCode, provinceCode, activityId);
