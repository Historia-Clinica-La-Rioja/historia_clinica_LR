...
public class SaveExternalPatientTest {
	...
	@BeforeEach
    public void setUp() {
        saveExternalPatient = new SaveExternalPatient(externalPatientStorage, patientInformationPublicApiPermission);
    }

    @Test
    void saveExistedExternalId() throws ExternalPatientBoException, ExternalPatientExtendedBoException {
        when(patientInformationPublicApiPermission.canAccessSaveExternalPatient()).thenReturn(true);
	...
	@Test
	void saveExternalPatientAccessDenied(){
		when(patientInformationPublicApiPermission.canAccessSaveExternalPatient()).thenReturn(false);
		Assertions.assertThrows(SaveExternalPatientAccessDeniedException.class, () -> {
			saveExternalPatient.run(validExternalPatientExtended());
		});
	}
